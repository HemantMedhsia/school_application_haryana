...
const sortNoticesByDate = (notices) => {
  return notices
    ...
    .slice(0, 2); // Show only the two most recent notices
};
            ...
            return (
              ...
              >
                <div className="flex justify-between items-center">
                  <h1 className={`text-xl font-semibold ${text}`}>{notice.title}</h1>
                  <span className={`px-2 py-1 text-sm font-bold rounded ${text} bg-opacity-20`}>
                    {notice.category}
                  </span>
                </div>
                <p className="mt-1 text-sm text-gray-400">
                  {parseDate(notice.date).toLocaleDateString()}
                </p>
              </li>
            );
          })}
        ...
        <div className="fixed z-50 inset-0 flex items-center justify-center bg-white bg-opacity-10">
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <h2 className={`text-2xl font-bold ${getCategoryStyles(selectedNotice.category).text}`}>
                {selectedNotice.title}
              </h2>
              <span className={`px-2 py-1 text-sm font-bold rounded ${getCategoryStyles(selectedNotice.category).text} bg-opacity-20`}>
